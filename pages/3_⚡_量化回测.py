...
from common import show_disclaimer, plot_equity_curve
...
st.markdown("### 📋 股票池")
pool_choice = st.selectbox("选择股票池", ["沪深300 (推荐)", "中证500", "自定义代码"], index=0)
custom_codes = ""
    ...
    if start_date >= end_date:
        ...
        with st.spinner(f"回测运行中 ({sd} ~ {ed})..."):
            try:
                ...
                metrics = run_backtest(codes, sd, ed, initial_cash=initial_cash,
                                       min_score=min_score, max_positions=max_positions,
                                       position_pct=position_pct, hold_days=hold_days,
                                       stop_loss=stop_loss, take_profit=take_profit)
                if 'error' in metrics:
                    st.error(metrics['error'])
                    st.info("💡 请先在「集合竞价」页面运行扫描生成数据")
                else:
                    st.success("回测完成!")
                    st.markdown("### 📊 回测结果")
