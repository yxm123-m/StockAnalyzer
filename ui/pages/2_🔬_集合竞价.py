...
from strategy.filters import filter_stocks
from ui.components.common import show_disclaimer, score_color, plot_bar
...
if scan_btn:
    ...
    with st.spinner("正在获取集合竞价数据..."):
        ...
        if ca_data is None or ca_data.empty:
            st.error("无法获取竞价数据。可能原因：非交易日、akshare接口异常、或数据源变动。")
            ...
            if not df_show.empty:
                df_show = df_show[['rank', 'code', 'name', 'total_score', 'grade',
                                    'volume_score', 'trend_score', 'imbalance_score',
                                    'gap_score', 'open_gap_pct']]
                df_show.columns = ['排名', '代码', '名称', '总分', '等级',
                                   '量能分', '趋势分', '失衡分', '跳空分', '跳空%']
                ...
                st.dataframe(
                    ...
                    hide_index=True,
                    column_config={
                        '总分': st.column_config.NumberColumn(format='%.1f'),
                        '跳空%': st.column_config.NumberColumn(format='%.2f%%'),
                    }
                )
            ...
            fig = plot_bar(hist_data.index.tolist(), hist_data.values.tolist(),
...
if st.button("📂 加载最近信号"):
    ...
    if signals:
        ...
        cols_show = ['trade_date', 'code', 'name', 'total_score', 'grade', 'rank', 'open_gap_pct']
        df_sig = df_sig[[c for c in cols_show if c in df_sig.columns]]
